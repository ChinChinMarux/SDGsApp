...
const AnalisisContent = ({ isDarkMode = false }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/analysis/status/${status.analysisId}`);
  ...
  // UI Components
  const renderSelect = ({ label, value, onChange, onClear, options, renderOption }) => (
    <Box mb={isMobile ? 2 : 3}>
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        {/* Error Alert */}
        {status.error && (
          <Alert severity="error" sx={{ mb: 3, fontSize: isMobile ? '0.8125rem' : '0.875rem' }}>
            {status.error}
          </Alert>
        )}
              ...
              {status.loading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip />
                    <Bar dataKey="weight" fill={colors.primary} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <Box sx={{
                  ...
                  Belum ada data distribusi topik. Silakan jalankan analisis terlebih dahulu.
                </Box>
              )}
            </Box>
          </Paper>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 1.5 : 2 }}>
              {status.loading ? (
                  ...
                  <Box key={index} mb={isMobile ? 1.5 : 2}>
                    <Box sx={{
                      ...
                      <Typography sx={{ 
                        ...
                        {isMobile ? `${item.sdg}:` : `${item.sdg}: ${item.description}`}
                      </Typography>
                      <Typography sx={{ 
                        fontWeight: 600,
                        fontSize: isMobile ? '0.875rem' : '0.9375rem'
                ...
                <Box sx={{
                  ...
                  Belum ada hasil pemetaan SDG. Silakan jalankan analisis terlebih dahulu.
                </Box>
              )}
            </Box>
          </Paper>
