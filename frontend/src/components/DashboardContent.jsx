...
const KnowledgeGraph = ({ isdarkmode, isMobile, data = [] }) => {
  ...
  useEffect(() => {
    ...
    const scale =
      Math.min(dimensions.width, dimensions.height) / (isMobile ? 250 : 300);
...
const IconDropdown = ({
  ...
  const getCorpusIcon = (corpusId) => {
    const iconMap = {
      climate: (
        <EnergySavingsLeafOutlined fontSize={isMobile ? "small" : "medium"} />
      ),
        ...
        {!isMobile && (
          <>
            <span style={{ flex: 1, textAlign: "left" }}>
              {analyses.find((a) => a.id === selectedCorpus)?.name ||
                "Select Analysis"}
            </span>
            <ArrowDownIcon
              fontSize="small"
        ...
        >
          {analyses.map((analysis) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ marginBottom: "2px" }}>{analysis.name}</div>
                <div
                  style={{
...
const DashboardContent = ({ isdarkmode = false }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      const formattedAnalyses = Object.values(allAnalysesData).map(
        (corpus) => ({
          id: corpus.id,
          name: corpus.name,
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {showAllAnalyses
              ? `Semua Analisis - ${currentAnalysis.name}`
              : "Analisis Terbaru"}
          </h3>
            ...
            {showAllAnalyses && (
              <button
                onClick={() => {
                  ...
                  alert(
                    `Mengexport ${filteredResults.length} analisis dari ${currentAnalysis.name}`
                  );
                }}
                style={{
                  padding: "10px 16px",
        ...
        {showAllAnalyses && (
          ...
          >
            Menampilkan {filteredResults.length} dari{" "}
            {currentAnalysis?.analysis_count || 0} analisis dalam korpus ini
          </div>
        )}
      </div>
    </div>
